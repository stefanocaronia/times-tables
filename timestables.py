...
while retry and (retry.upper()[0] == 'S' or retry.upper()[0] == 'Y'):
    ...
    players = ', '.join(names)
    if vars.player == '':
        vars.player = functions.cinput_translate("Hello! Who are you" + (" (" + players + ")" if len(players) > 0 else "") + "? ", Fore.WHITE)
    ...
    vars.single = functions.cinput_translate("With which timetable do you want to play, " + vars.player + " (return for all)? ", Fore.WHITE)

    if not functions.is_numeric(vars.single) or int(vars.single) < 2:
        vars.max = functions.cinput_translate("Up to which times table did you study (return for 9)? ", Fore.WHITE)
    ...
    functions.print_translate(Fore.WHITE + "You have " + Fore.CYAN + str(vars.QUESTIONS) + Fore.WHITE + " questions and " + Fore.CYAN + str(vars.lives) + Fore.WHITE + " lives" + Fore.WHITE)
    print(" ")
    ...
    while (vars.answers < vars.QUESTIONS and vars.lives > 0):
        ...
        while ([num1, num2] in already or [num2, num1] == last or [num2, num1] in already) and tries < 10:
            num1 = int(vars.single) if functions.is_numeric(vars.single) else functions.get_random_number(int(vars.max))
        ...
        while answer == '':
            ...
            answer = functions.cinput_translate("How much is " + operation + "? ", Fore.WHITE)
        ...
        if int(answer) == result:
            ...
            functions.print_translate(Fore.GREEN + indent + "Good! The result is correct! " + Style.BRIGHT + operation + Fore.WHITE + Style.NORMAL)
            functions.print_translate(Fore.GREEN + indent + "You answered in " + str(elapsed) + " seconds " + Fore.WHITE)
            functions.print_translate(Fore.GREEN + indent + "You got " + str(prize) + " points " + Fore.WHITE)
            if bonus > 0:
                functions.play('sounds/applause.wav')
                functions.print_translate(Fore.YELLOW + indent + "Hurray! You had a bonus of " + str(bonus) + " points " + Fore.WHITE)
            else:
        ...
        if vars.lives > 0 and vars.answers < vars.QUESTIONS:
            functions.status(vars.score, vars.lives, vars.QUESTIONS - vars.answers)
            functions.print_translate(indent + "Press a key for the next question")
            functions.wait()
            print('')

    functions.endgame()

    retry = functions.cinput_translate("Do you want to play another game? ", Fore.WHITE)
